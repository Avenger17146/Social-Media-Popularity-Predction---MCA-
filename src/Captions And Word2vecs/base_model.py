...
class BaseModel(object):
    ...
    def test(self, sess, test_data, vocabulary):
        ...
        for k in tqdm(list(range(test_data.num_batches)), desc='path'):
            ...
            for l in range(test_data.batch_size-fake_cnt):
                ...
                caption = vocabulary.get_sentence(word_idxs)
                # print(caption)
                nuguntru = caption.split('.')
                nuguntru = nuguntru[0]
                nuguntru = nuguntru.split()

                captions.append(caption)
                ris  = np.zeros(np.shape(self.w2v['on'])[0])
                count  = 0
                for goo in nuguntru : 
                    # print(goo)
                    count+=1
                ...
                image_name = image_file.split(os.sep)[-1]
                image_name = os.path.splitext(image_name)[0]
                pid.append(image_name.split('/')[3])
                img = plt.imread(image_file)
                plt.imshow(img)
                plt.axis('off')
                plt.title(caption)
                plt.savefig('./test/results'+str(l)+'_result.jpg')

        # Save the captions to a file
        results = pd.DataFrame({'pid':pid, 'w2v':w2v_score,
                                'caption':captions,
                                'prob':scores})
    ...
    def beam_search(self, sess, image_files, vocabulary):
        ...
        # a, b, c = sess.run("conv5_3", feed_dict = {self.images: images})
        # print(a)
        # print(b)
        # print(c)
